...
from models.base_sharing import DownsampleB, SeparableConv2d
...
class FlatResNet26(FlatResNet):
	...
	self.cnt_depth = 0
        ...
        self.conv1 = SeparableConv2d(3, int(64*factor), 3, 1, 1, 1, False, self.nb_tasks, self.cnt_depth)
        self.cnt_depth += 1
        #self.pre_bn = nn.ModuleList([nn.BatchNorm2d(int(64*factor)) for i in range(self.nb_tasks)])
        self.pre_bn = nn.ModuleList([nn.BatchNorm2d(int(64*factor))])
        ...
        for idx, (filt_size, num_blocks, stride) in enumerate(zip(filt_sizes, layers, strides)):
            blocks, ds = self._make_layer(block, filt_size, num_blocks, stride, self.cnt_depth)
        ...
        self.bns = nn.ModuleList([nn.Sequential(nn.BatchNorm2d(int(512*factor)), nn.ReLU(True))])
        self.avgpool = nn.AdaptiveAvgPool2d(1) 
        #self.fcs = nn.ModuleList([nn.Linear(int(512*factor), num_classes[i]) for i in range(self.nb_tasks)])         
        self.fcs = nn.ModuleList([nn.Linear(int(512*factor), num_classes[0])])         
    ...
    def _make_layer(self, block, planes, blocks, stride=1, cnt_depth=0):

        downsample = nn.Sequential()
        if stride != 1 or self.in_planes != planes:
            downsample = DownsampleB(self.in_planes, planes, 2)

        layers = [block(self.in_planes, planes, self.nb_tasks, stride, self.cnt_depth)]
        self.cnt_depth += 2

        self.in_planes = planes 
        for i in range(1, blocks):
            layers.append(block(self.in_planes, planes, self.nb_tasks, cnt_depth = self.cnt_depth))
            self.cnt_depth += 2
