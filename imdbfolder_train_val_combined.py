...
def prepare_data_loaders(dataset_names, data_dir, imdb_dir, shuffle_train=True, index=None):
	...
	if dataset_names[i] in ['gtsrb', 'omniglot','svhn']: # no horz flip 
        ...
        valloader = torch.utils.data.DataLoader(ImageFolder(data_dir, transform_test, None, None, labels_val, imgnames_val), batch_size=50, shuffle=False, num_workers=4, pin_memory=True)
